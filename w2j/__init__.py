...
if os.environ.get('W2J_TEST_MODE') != '1':
    ...
    parser.add_argument('--force', '-f', action='store_true', help='强制重新迁移所有笔记，无视迁移进度。会覆盖已有笔记。')
...
def main() -> None:
    ...
    if args.target == 'obsidian':
        ...
        ad = adapter.ObsidianAdapter(ws, vault_path, work_dir=output_dir, force=args.force)
        try:
            if args.all:
                ad.sync_all()
            else:
